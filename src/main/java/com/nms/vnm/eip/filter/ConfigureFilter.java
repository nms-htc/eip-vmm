 ...
 * @author CuongNT
 */
@WebFilter(filterName = "Configure Filter", urlPatterns = {"/*"}, dispatcherTypes = {DispatcherType.FORWARD, DispatcherType.REQUEST})
public class ConfigureFilter implements Filter {
 ...
 public void doFilter(ServletRequest request, ServletResponse response,
  ...
  try {
   ...
   UserAgentInfo agentInfo = new UserAgentInfo(userAgent, accept);

   if ((agentInfo.isAndroidPhone || agentInfo.isIphone) && uri.contains("/feature/")) {
    uri = uri.replace("/feature/", "/smart/");
    httpServletResponse.sendRedirect(uri);
   } else if (agentInfo.isMobileDevice() && (!agentInfo.isAndroidPhone && !agentInfo.isIphone) && uri.contains("/smart/")) {
    uri = uri.replace("/smart/", "/feature/");
    httpServletResponse.sendRedirect(uri);
   } else {
    chain.doFilter(request, response);
   }
