...
import com.nms.vnm.eip.entity.User_;
...
import com.nms.vnm.eip.web.util.StringUtil;
            ...
            case "groups":
                if (entry.getValue() != null) {
                    ...
                    for (int i = 0; i < groups.length; i++) {
                        predicates[i] = cb.isMember((User.Group)groups[i], root.get(User_.groups));
                    }
                    predicate = cb.or(predicates);
                }
                break;
            case "description":
                predicate = cb.like(cb.upper(root.get(User_.description)), "%" + entry.getValue().toString().toUpperCase()  + "%");
                break;
            case "code":
                predicate = cb.like(cb.upper(root.get(User_.code)), "%" + entry.getValue().toString().toUpperCase()  + "%");
                break;
        }
        return predicate;
    }
